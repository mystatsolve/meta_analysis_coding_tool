...
JSON 구조:
{
  "study": {
    ...
    "journal": "저널명",
    "title": "논문 제목",
    "study_design": "two-group-pre-post | two-group-post-only | one-group-pre-post | repeated-measures | other",
    "n_TG": 실험/훈련군 n (정수),
    "n_CG": 통제/대조군 n (정수, 통제군 없으면 null),
    "population": "연구대상 설명",
    "intervention": "중재 프로그램 설명",
    "duration": "훈련/개입 기간",
    "measurement_note": "측정 시점 설명 (예: 사전-사후 2회, 사전-중간-사후 3회, 주 단위 반복 등)"
  },
  "outcomes": [
    {
      ...
      "TG_pre_M":   실험군 사전(첫 번째 측정) 평균  (숫자 또는 null),
      "TG_pre_SD":  실험군 사전(첫 번째 측정) SD   (숫자 또는 null),
      "TG_post_M":  실험군 사후(마지막 측정) 평균  (숫자 또는 null),
      "TG_post_SD": 실험군 사후(마지막 측정) SD   (숫자 또는 null),
      "CG_pre_M":   통제군 사전(첫 번째 측정) 평균  (숫자 또는 null, 통제군 없으면 null),
      "CG_pre_SD":  통제군 사전(첫 번째 측정) SD   (숫자 또는 null, 통제군 없으면 null),
      "CG_post_M":  통제군 사후(마지막 측정) 평균  (숫자 또는 null, 통제군 없으면 null),
      "CG_post_SD": 통제군 사후(마지막 측정) SD   (숫자 또는 null, 통제군 없으면 null),
      "F_group":        집단 간 F값    (숫자 또는 null),
      "F_time":         시간 F값       (숫자 또는 null),
      "F_interaction":  상호작용 F값   (숫자 또는 null),
      "note": "통계 유의성, 측정 시점 정보 및 특이사항 (예: Time p<0.01; G×T p<0.05; 3회 반복측정 중 1차·3차 사용)"
    }
  ]
}
...
3. 실험군(TG): 훈련군·실험군·운동군·EG·TG·치료군 등
   통제군(CG): 통제군·대조군·CG·비교군·WLC 등 / **통제군이 없으면** CG 필드는 모두 null.
4. **반복 측정 (3회 이상)**: T1·T2·T3·... 또는 Pre·Mid·Post 등 여러 시점이 있으면
   - TG_pre_M/SD = 가장 첫 번째 측정값 (T1 또는 Pre)
   - TG_post_M/SD = 가장 마지막 측정값 (T3, Post 등)
   - 중간 측정값은 note 필드에 기록하되 pre/post에는 포함하지 않음.
5. **study_design 값**:
   - "two-group-pre-post": 실험군+통제군, 사전-사후 2회 측정
   - "two-group-post-only": 실험군+통제군, 사후만 측정 (사전값 없음) → TG_pre/CG_pre는 null
   - "one-group-pre-post": 실험군만, 사전-사후 2회 측정
   - "repeated-measures": 3회 이상 반복 측정 (집단 수 무관)
   - "other": 위 분류에 맞지 않는 경우
6. **사후 비교만 있는 경우 (two-group-post-only)**:
   - TG_pre_M/SD, CG_pre_M/SD는 null로 설정
   - TG_post_M/SD, CG_post_M/SD에 실험군·통제군의 측정값 입력
   - 논문에서 "비교군" "대조군" 평균을 사후 값으로 처리
6. F값의 *, ** 표시는 note에 포함 (예: "F=9.65, p<0.01").
7. 값 없으면 null. 측정치 없는 변수도 null로 포함.
8. 서브그룹은 논문 결과 섹션 구분을 따름.
"""
...
def calc_effects(outcomes: list, n_tg: int, n_cg: int):
    ...
    설계 유형에 따라 공식 자동 선택 (데이터 가용성 기반 자동 감지):

    ① 두 집단 사전-사후 (n_cg > 0, pre+post 모두 있음) — Morris (2008):
        Cohen's d = (Δ_TG - Δ_CG) / SD_pooled_pre
            Δ = post_M - pre_M
            SD_pooled_pre = sqrt(((n_TG-1)*SD_TG_pre² + (n_CG-1)*SD_CG_pre²) / df)
            df = n_TG + n_CG - 2

    ② 두 집단 사후 비교 (n_cg > 0, post만 있고 pre 없음) — Cohen (1988):
        Cohen's d = (M_TG_post - M_CG_post) / SD_pooled_post
            SD_pooled_post = sqrt(((n_TG-1)*SD_TG_post² + (n_CG-1)*SD_CG_post²) / df)
            df = n_TG + n_CG - 2

    ③ 단일군 사전-사후 (n_cg == 0) — Cohen (1988) / Becker (1988):
        Cohen's d = Δ_TG / SD_TG_pre
            df = n_TG - 1

    모든 경우: Hedges' g = d × J,  J = 1 - 3/(4×df - 1)

    매개변수:
        outcomes (list): AI 추출 결과 딕셔너리 목록
        n_tg (int): 훈련군(실험군) 표본 크기
        n_cg (int): 통제군 표본 크기 (통제군 없으면 0)

    반환값:
        tuple: (업데이트된 outcomes 리스트, Hedges' J 보정값, 자유도 df)
    """
    is_one_group = (n_cg == 0)  # 통제군 없으면 단일군 설계

    if is_one_group:
        df_val = max(n_tg - 1, 1)    # 단일군: df = n - 1
    else:
        df_val = n_tg + n_cg - 2     # 두 집단: df = n_TG + n_CG - 2

    J = 1 - 3 / (4 * df_val - 1)    # Hedges' g 소표본 보정 계수

    for o in outcomes:

        if is_one_group:
            # ── ① 단일군: TG_pre/post 만 필요 ────────────────────────────────
            vals = [o.get(k) for k in ["TG_pre_M", "TG_pre_SD", "TG_post_M"]]
            if any(v is None for v in vals):
                o.update(delta_TG=None, delta_CG=None,
                         SD_pooled_pre=None, cohen_d=None, hedges_g=None)
                continue
            try:
                tg_pre_m, tg_pre_sd, tg_post_m = vals
                delta_tg = tg_post_m - tg_pre_m       # 실험군 변화량
                sp = tg_pre_sd                         # 사전 SD를 분모로 사용
                d = delta_tg / sp if sp > 0 else None
                g = d * J if d is not None else None
                o.update(
                    delta_TG=round(delta_tg, 4),
                    delta_CG=None,                     # 통제군 없음
                    SD_pooled_pre=round(sp, 2),
                    cohen_d=round(d, 2) if d is not None else None,
                    hedges_g=round(g, 2) if g is not None else None,
                )
            except Exception:
                o.update(delta_TG=None, delta_CG=None,
                         SD_pooled_pre=None, cohen_d=None, hedges_g=None)
            ...
            # ── 두 집단 설계: 데이터 가용성에 따라 공식 자동 선택 ─────────────

            # 우선순위 ①: 사전-사후 모두 있으면 Morris (2008)
            pre_post_vals = [o.get(k) for k in
                             ["TG_pre_M", "TG_pre_SD", "TG_post_M",
                              "CG_pre_M", "CG_pre_SD", "CG_post_M"]]

            if not any(v is None for v in pre_post_vals):
                # ── ① 두 집단 사전-사후: Morris (2008) ───────────────────────
                try:
                    tg_pre_m, tg_pre_sd, tg_post_m, cg_pre_m, cg_pre_sd, cg_post_m = pre_post_vals
                ...
                # 우선순위 ②: 사전 없고 사후만 있으면 독립표본 사후 비교
                post_only_vals = [o.get(k) for k in
                                  ["TG_post_M", "TG_post_SD", "CG_post_M", "CG_post_SD"]]

                if not any(v is None for v in post_only_vals):
                    # ── ② 두 집단 사후 비교: Cohen (1988) 독립표본 ───────────
                    try:
                        tg_post_m, tg_post_sd, cg_post_m, cg_post_sd = post_only_vals
                        sp = math.sqrt(
                            ((n_tg - 1) * tg_post_sd ** 2 +
                             (n_cg - 1) * cg_post_sd ** 2) / df_val
                        )
                        d = (tg_post_m - cg_post_m) / sp if sp > 0 else None
                        g = d * J if d is not None else None
                        o.update(
                            delta_TG=None,           # 변화량 없음(사전 없음)
                            delta_CG=None,
                            SD_pooled_pre=round(sp, 2),  # 실제론 사후 기반 pooled SD
                            cohen_d=round(d, 2) if d is not None else None,
                            hedges_g=round(g, 2) if g is not None else None,
                        )
                    except Exception:
                    ...
                    # 우선순위 ③: TG 사전-사후만 있으면 단일군 방식으로 fallback
                    tg_pre_post = [o.get(k) for k in ["TG_pre_M", "TG_pre_SD", "TG_post_M"]]
                    if not any(v is None for v in tg_pre_post):
                        try:
                            tg_pre_m, tg_pre_sd, tg_post_m = tg_pre_post
                            delta_tg = tg_post_m - tg_pre_m
                            sp = tg_pre_sd
                            df_fb = max(n_tg - 1, 1)
                            J_fb = 1 - 3 / (4 * df_fb - 1)
                            d = delta_tg / sp if sp > 0 else None
                            g = d * J_fb if d is not None else None
                            o.update(
                                delta_TG=round(delta_tg, 4),
                                delta_CG=None,
                                SD_pooled_pre=round(sp, 2),
...
def to_dataframe(study: dict, outcomes: list) -> pd.DataFrame:
    ...
    _design_map_df = {
        "two-group-pre-post":  "양군 사전-사후",
        "two-group-post-only": "양군 사후비교",
        "one-group-pre-post":  "단일군 사전-사후",
        "repeated-measures":   "반복측정",
        "other":               "기타",
    }
    _design_label = _design_map_df.get(
        study.get("study_design", ""), study.get("study_design", "")
    )
    ...
    for o in outcomes:  # 각 결과 변수(outcome)마다 1개 행 생성
        rows.append({
            ...
            "n_CG": study.get("n_CG") or "없음(단일군)",             # 통제군 표본 크기
...
{{
  ...
  "study_check": {{
    ...
    "n_CG": {{"status": "ok"|"error"|"uncertain"|"not_applicable", "original": <원본값 또는 null>, "verified": <검증값 또는 null>, "note": "통제군 없으면 not_applicable"}}
  }},
  "outcomes_check": [
    {{
      "outcome_en": "결과변수 영문명",
      ...
      "checks": {{
        "TG_pre_M":   {{"status": "ok"|"error"|"uncertain", "original": <숫자 또는 null>, "verified": <숫자 또는 null>, "note": "근거 위치 및 측정 시점 (예: Table 2, T1=사전)"}},
        "TG_pre_SD":  {{"status": "...", "original": ..., "verified": ..., "note": "..."}},
        "TG_post_M":  {{"status": "...", "original": ..., "verified": ..., "note": "근거 위치 및 측정 시점 (예: Table 2, T3=사후)"}},
        "TG_post_SD": {{"status": "...", "original": ..., "verified": ..., "note": "..."}},
        "CG_pre_M":   {{"status": "ok"|"error"|"uncertain"|"not_applicable", "original": ..., "verified": ..., "note": "통제군 없으면 not_applicable"}},
...
- "not_applicable": 해당 없음 (통제군 없는 단일군 연구의 CG 필드, 사후 비교 연구의 pre 필드 등)
- 반복 측정(3회 이상): TG_pre = 첫 번째 측정, TG_post = 마지막 측정이 맞는지 확인
- 사후 비교 연구(pre 없음): TG_pre/CG_pre는 not_applicable, TG_post/CG_post 수치가 올바른지 확인
"""
        ...
        for o in corrected:
            if o.get("outcome_en", "") == en:  # 일치하는 결과 변수 찾기
                for field in NUMERIC_FIELDS:
                    ...
                    # 오류이고 검증값이 있는 경우에만 수정 (not_applicable은 건드리지 않음)
                    if (chk.get("status") == "error"
                            and chk.get("verified") is not None
                            and chk.get("status") != "not_applicable"):
...
with st.sidebar:
    ...
    st.markdown("**① 양군 사전-사후 (Morris, 2008)**")
    st.latex(r"d = \frac{\Delta_{TG} - \Delta_{CG}}{SD_{pooled,pre}}")
    st.markdown("**② 양군 사후 비교 (Cohen, 1988 독립표본)**")
    st.latex(r"d = \frac{M_{TG,post} - M_{CG,post}}{SD_{pooled,post}}")
    st.markdown("**③ 단일군 사전-사후 (Cohen, 1988)**")
    st.latex(r"d = \frac{\Delta_{TG}}{SD_{TG,pre}}")
    st.markdown("**Hedges' g (소표본 보정, 공통)**")
    st.latex(r"g = d \times \left(1-\frac{3}{4df-1}\right)")
    st.caption("공식은 데이터 가용성에 따라 자동 선택됩니다.\n통제군 n=0이면 ③이 적용됩니다.")
...
| 유형 | 설명 | 효과크기 공식 |
|------|------|--------------|
| 양군 사전-사후 | 실험군+통제군, 사전+사후 측정 | Morris (2008) |
| 양군 사후비교 | 실험군+통제군, 사후 측정만 | Cohen (1988) 독립표본 |
| 단일군 사전-사후 | 실험군만, 사전+사후 측정 (통제군 n=0) | Cohen (1988) |
| 반복 측정 (3회↑) | 3회 이상 측정 (첫·마지막 사용) | 위 공식 자동 선택 |
...
_n_cg_display = study.get("n_CG")
m4.metric("통제군 n", _n_cg_display if _n_cg_display else "없음(단일군)")
# 연구 설계 유형 표시
_design_map = {
    "two-group-pre-post":   "양군 사전-사후",
    "two-group-post-only":  "양군 사후비교",
    "one-group-pre-post":   "단일군 사전-사후",
    "repeated-measures":    "반복 측정",
    "other":                "기타",
}
_design_raw = study.get("study_design", "")
m5.metric("연구 설계", _design_map.get(_design_raw, _design_raw or "N/A"))
...
# value: AI 추출값을 기본값으로
_tg_default = max(1, int(study.get("n_TG") or 1))
_cg_raw = study.get("n_CG")
_cg_default = int(_cg_raw) if _cg_raw else 0  # 통제군 없으면 0

n_tg = nc1.number_input("훈련군(TG) n", value=_tg_default, min_value=1)
n_cg = nc2.number_input(
    "통제군(CG) n",
    value=_cg_default,
    min_value=0,   # 0 = 통제군 없음 (단일군 설계)
    help="통제군이 없으면 **0**을 입력하세요. 단일군 사전-사후 효과크기 공식이 자동 적용됩니다.",
)

# 자유도 및 Hedges' J 실시간 계산 + 설계 유형 안내
if n_cg == 0:
    df_val = max(n_tg - 1, 1)
    J_val  = 1 - 3 / (4 * df_val - 1)
    st.caption(
        f"📐 설계: **단일군 사전-사후** (Cohen, 1988)  ·  "
        f"d = Δ_TG / SD_TG_pre  ·  df = {df_val}  ·  Hedges' J = {J_val:.4f}"
    )
else:
    df_val = n_tg + n_cg - 2
    J_val  = 1 - 3 / (4 * df_val - 1)
    _detected_design = study.get("study_design", "")
    if _detected_design == "two-group-post-only":
        st.caption(
            f"📐 설계: **양군 사후 비교** (Cohen, 1988 독립표본)  ·  "
            f"d = (M_TG − M_CG) / SD_pooled_post  ·  df = {df_val}  ·  Hedges' J = {J_val:.4f}"
        )
        st.info("ℹ️ 사전값이 없는 경우 사후 평균 차이를 합동 SD로 나눈 독립표본 Cohen's d가 계산됩니다.")
    else:
        st.caption(
            f"📐 설계: **양군 사전-사후** (Morris, 2008)  ·  "
            f"d = (Δ_TG − Δ_CG) / SD_pooled_pre  ·  df = {df_val}  ·  Hedges' J = {J_val:.4f}  "
            f"| 사전값 없는 변수는 자동으로 사후 비교 공식 적용"
        )
...
_default_cols = [
    "outcome_kr", "subgroup_kr", "단위", "연구설계",
    ...
    if not fc_df.empty:
        ...
        def _highlight_status(row):
            ...
            if "오류" in s:
                ...
                return ["background-color: #F5F5F5"] * len(row)   # 연회색
            return [""] * len(row)
